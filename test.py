...
st.markdown(
    ...
    <style>
      ...
      /* 2) Supprime le padding interne daaaaau bloc qui contient vos onglets + contenu */
      .block-container {
...
conn_emp  = get_conn_employes(); cur_emp  = conn_emp.cursor()
conn_form = get_conn_formations(); cur_form = conn_form.cursor()
conn_prog = get_conn_progress(); cur_prog = conn_prog.cursor()
conn_test = get_conn_tests();     cur_test = conn_test.cursor()
...
def login_page():
    ...
    st.markdown(
        f"<h1>{t('🔐 Connexion','🔐 Log in','🔐 Iniciar sesión')}</h1>",
        unsafe_allow_html=True
    )

    # champs locaux, pas de st.session_state.assign ici
...
def creer_certificat(nom, formation, date_certif):
    ...
    if logo_path and os.path.exists(logo_path):
        ...
        x_center = (pdf.w - logo_w) / 2  # centrer sur la largeur de la page
        y_logo = 18  # position verticale du logo
        pdf.image(logo_path, x=x_center, y=y_logo, w=logo_w)
        # Supprimer le fichier temporaire après insertion
        os.remove(logo_path)
    ...
    texte = f"""
{t('Ce certificat est décerné à :','This certificate is awarded to:','Este certificado se otorga a:')}

{nom}

{t('Pour avoir suivi avec succès la formation :','For successfully completing the training:','Por haber completado con éxito la formación:')}

"{formation}"

{t('Délivré le :','Issued on:','Emitido el:')} {date_certif.strftime('%d/%m/%Y')}

{t('Ce certificat atteste de la participation active, de l\'assiduité et de l\'engagement','This certificate certifies active participation, regular attendance, and commitment','Este certificado certifica la participación activa, la asistencia regular y el compromiso')}
{t('dans le cadre d\'un programme de développement professionnel.','as part of a professional development program.','como parte de un programa de desarrollo profesional.')}
"""
...
def main():
    ...
    if role in ["Admin","RH"]:
        tabs = st.tabs([
            ...
            t("Paramètres","Settings","Configuración"),
    ...
    if role in ["Admin","RH"]:
        ...
        with tabs[0]:
            ...
            with col1:
                st.markdown(
                    f"<h2 style='text-align:center; font-size:18px; margin:0px 0;'>"
                    f"{t('➕ Ajouter une formation','➕ Add Training','➕ Agregar Formación')}"
                    "</h2>",
                    unsafe_allow_html=True
                )
                ...
                duree = st.number_input(t("Durée (h)","Duration (h)","Duración (h)"),
            ...
            with col2:
                st.markdown(
                    f"<h2 style='text-align:center; font-size:18px; margin:0px 0;'>"
                    f"{t("🛠 Modifier / Supprimer","🛠 Edit / Delete","🛠 Editar / Eliminar")}"
                    "</h2>",
                    unsafe_allow_html=True
                )
                ...
                if data:
                    ...
                    new_d = st.date_input(t("Date","Date","Fecha"), value=datetime.fromisoformat(old_d), key="mod_date")
                    new_du = st.number_input(t("Durée (h)","Duration (h)","Duración (h)"), value=old_du, min_value=1, step=1, key="mod_duree")
                    ...
                    with c_mod:
                        if st.button(t("Modifier","Edit","Editar"), key="mod_form_btn"):
                            ...
                            conn_form.commit()
                            st.success(t("Formation modifiée ✏️","Training updated ✏️","Formación actualizada ✏️"))
                            time.sleep(1); st.rerun()
                    with c_del:
                        if st.button(t("Supprimer","Delete","Eliminar"), key="del_form_btn"):
                            cur_form.execute("DELETE FROM formations WHERE id=?", (fid,))
                            conn_form.commit()
                            st.warning(t("Formation supprimée 🗑️","Training deleted 🗑️","Formación eliminada 🗑️"))
                            time.sleep(1); st.rerun()
                else:
            ...
            df_forms = pd.DataFrame(cur_form.fetchall(), columns=[t("Titre","Title","Título"),
                                                                  t("Date","Date","Fecha"),
        ...
        with tabs[1]:
            ...
            with col2:
                ...
                if emp_data:
                    ...
                    n_f_disp = st.selectbox(t("Fonction","Role","Rol"), list(fonctions_ocp.keys()),
                                             index=list(fonctions_ocp.values()).index(old_f), key="mod_fonct_emp")
            ...
            df_emp = pd.DataFrame(cur_emp.fetchall(), columns=[t("Nom","Last Name","Apellido"),
        ...
        # 3️⃣ Administration des chapitres
        with tabs[2]:
            ...
            if mode == t("Ajouter Chapitre","Add Chapter","Agregar Capítulo"):
                # ← ici on récupère TOUTES les formations
                fms2 = cur_form.execute(
                    "SELECT id,titre FROM formations ORDER BY date DESC"
                ).fetchall()

                # SI LA LISTE EST VIDE, on informe l'utilisateur et on sort
                if not fms2:
                    st.info(t(
                    ...
                    # Maintenant on est sûr que sel2 existe dans mapping_formations
                    ...
                    ch_order = st.number_input(t("Ordre","Order","Orden"), min_value=1, step=1, key="add2_ch_order")
                    ch_type = st.selectbox(t("Type de contenu","Content Type","Tipo de contenido"), ["texte","pdf","video","ppt"], key="add2_ch_type")
                    ch_content = None
                    if ch_type == "texte":
                        ch_content = st.text_area(t("Contenu texte","Text content","Contenido de texto"), key="add2_ch_content")
                    else:
                        up = st.file_uploader(t("Fichier","File","Archivo"), type={"pdf":["pdf"],"video":["mp4"],"ppt":["ppt","pptx"]}[ch_type], key="add2_ch_file")
                        if up:
                    ...
                    if st.button(t("Ajouter","Add","Agregar"), key="add2_ch_btn"):
                        if ch_title and ch_content:
                            cur_form.execute("SELECT COUNT(*) FROM chapitres WHERE formation_id=? AND titre=?", (fid2, ch_title))
                            if cur_form.fetchone()[0] > 0:
                                ...
                                conn_form.commit()
                                st.success(t("Chapitre ajouté ✅","Chapter added ✅","Capítulo agregado ✅"))
                                time.sleep(1); st.rerun()
                        else:
                    ...
                    if chap_list:
                        ...
                        new_t3 = st.text_input(t("Titre","Title","Título"), old_t3, key="mod2_ch_title")
                        new_ord3 = st.number_input(t("Ordre","Order","Orden"), min_value=1, value=old_ord3, step=1, key="mod2_ch_order")
                        new_type3 = st.selectbox(t("Type de contenu","Content Type","Tipo de contenido"), ["texte","pdf","video","ppt"],
                                                index=["texte","pdf","video","ppt"].index(old_type3), key="mod2_ch_type")
                        if new_type3 == "texte":
                            new_cont3 = st.text_area(t("Contenu texte","Text content","Contenido de texto"), old_cont3, key="mod2_ch_content")
                        else:
                            nf = st.file_uploader(t("Fichier","File","Archivo"), type={"pdf":["pdf"],"video":["mp4"],"ppt":["ppt","pptx"]}[new_type3], key="mod2_ch_file")
                            if nf:
                                os.makedirs("uploads", exist_ok=True)
                                np = os.path.join("uploads", nf.name)
                                with open(np,"wb") as f: f.write(nf.read())
                                new_cont3 = np
                            else:
                                new_cont3 = old_cont3
                        c1, c2 = st.columns(2)
                        with c1:
                            if st.button(t("Modifier","Edit","Editar"), key="mod2_ch_btn"):
                                ...
                                conn_form.commit()
                                st.success(t("Chapitre modifié ✅","Chapter updated ✅","Capítulo actualizado ✅"))
                                time.sleep(1); st.rerun()
                        with c2:
                            if st.button(t("Supprimer","Delete","Eliminar"), key="del2_ch_btn"):
                                cur_form.execute("DELETE FROM chapitres WHERE id=?", (cid3,))
                                conn_form.commit()
                                st.warning(t("Chapitre supprimé 🗑️","Chapter deleted 🗑️","Capítulo eliminado 🗑️"))
                                time.sleep(1); st.rerun()
                    else:
                    ...
                    if not fms:
                        ...
                        num_opts = st.number_input(t("Nb options","# options","# opciones"), min_value=2, max_value=6, value=4, step=1, key="q_num_opts")
                        opts=[]; corrs=[]
                        for i in range(num_opts):
                            ...
                            opts.append(t_opt); corrs.append(c_opt)
                        if st.button(t("Ajouter","Add","Agregar"), key="add_q_btn"):
                            cur_test.execute("INSERT INTO questions(formation_id,question_text,allow_multiple) VALUES(?,?,?)",
                                            (fid_test, q_text, int(allow_multi)))
                            qid = cur_test.lastrowid
                            for t_opt, c_opt in zip(opts, corrs):
                                cur_test.execute("INSERT INTO options(question_id,option_text,is_correct) VALUES(?,?,?)",
                                                (qid, t_opt, int(c_opt)))
                            conn_test.commit()
                            st.success(t("Question ajoutée ✅","Question added ✅","Pregunta agregada ✅"))

        # 4️⃣ Gestion Utilisateur
        with tabs[3]:
            
            # — Votre code existant inchangé —
            st.markdown(
                f"<h1 style='text-align:center; font-size:28px; margin:0px;'>"
                f"{t('👤 Gestion Utilisateur','👤 User Management','👤 Gestión Usuarios')}"
                "</h1>",
                unsafe_allow_html=True
            )
            ...
            with col1:
                ...
                role_input = st.selectbox(t("Rôle","Role","Rol"), [t("Admin","Admin","Admin"), t("RH","HR","RRHH"), t("Utilisateur","User","Usuario")])
            ...
            with col2:
                ...
                if st.button(t("Mettre à jour","Update","Actualizar"), key="update_user"):
                    if not all([
                        ...
                        conn_users.commit()
                        st.success(
                            t("Profil mis à jour ✅","Profile updated ✅","Perfil actualizado ✅")
                        )
                        st.rerun()

            # — Tableau & suppression —
            cur_users.execute(
                "SELECT email,nom,prenom,fonction,genre FROM utilisateurs ORDER BY email"
            )
            ...
            if not df_users.empty:
                ...
                with col_delete:
                    st.subheader(t("🗑️ Supprimer un utilisateur","🗑️ Delete a user","🗑️ Eliminar usuario"))
        ...
        with tabs[4]:
            with st.sidebar:
                ...
                # Langue
                lang = st.selectbox(t("Langue","Language","Idioma"),
        ...
        with tabs[5]:
         # Scoped CSS pour le dashboard
            ...
            st.markdown(
                f"<h1 style='text-align:center; font-size:28px; margin:0px;padding: 0px;'>"
                f"{t("📊 Tableau de bord","📊 Dashboard","📊 Tablero")}"
                "</h1>",
                unsafe_allow_html=True
            )
            ...
            # Define KPIs with chart and chart title
            items = [
            {
            ...
            # Display in 3×3 grid with centered chart titles
        ...
        with tabs[0]:
            ...
            if not choix:
                ...
                fid = [f[0] for f in forms if f[1] == sel][0]
                # On récupère les chapitres
                chs = cur_form.execute(
                ...
                if total == 0:
                    ...
                    # On s'assure que idx reste dans les bornes
                    ...
                    for i in range(total):
                        ...
                        if i == idx: status = "step-current"
                        cols[i].markdown(
                            f"<div style='width:36px;height:36px;border-radius:50%;"
                            f"background-color:{'#2c6e49' if status=='step-done' else '#e47157' if status=='step-current' else '#cfcfcf'};"
                            f"display:flex;align-items:center;justify-content:center;color:white;'>{i+1}</div>",
                            unsafe_allow_html=True
                        )

                    # Déballage en toute sécurité
                    cid, tit, type_c, cont = chs[idx]
...
# Mes certificats
    ...
    if not passed:
        ...
        st.write("<div style='height: 300px;'></div>", unsafe_allow_html=True)  # espace pour remplir
    else:
        for fidc in passed:
        ...
        with tabs[3]:
            ...
            notif_form  = st.checkbox(t("Formations","Trainings","Formaciones"),
                                    value=(get_param("notif_form","True")=="True"),
                                    key="param_notif_form")
            notif_test  = st.checkbox(t("Tests","Tests","Pruebas"),
                                    value=(get_param("notif_test","True")=="True"),
                                    key="param_notif_test")
            notif_cert  = st.checkbox(t("Certificats","Certificates","Certificados"),
                                    value=(get_param("notif_cert","True")=="True"),
                                    key="param_notif_cert")
            ...
            with st.expander(t("❓ Comment créer un compte ?","❓ How to create an account?","❓ ¿Cómo crear una cuenta?")):
                st.write(t("→ Allez dans l’onglet “👤 Utilisateurs” et remplissez le formulaire.","→ Go to the “👤 Users” tab and fill out the form.","→ Ve al apartado “👤 Usuarios” y completa el formulario."))
            ...
            with st.expander(t("❓ Comment changer ma photo de profil ?","❓ How to change my profile photo?","❓ ¿Cómo cambiar mi foto de perfil?")):
                st.write(t(
                    "Dans l’onglet “👤 Utilisateurs”, cliquez sur “Changer la photo de profil”, choisissez un fichier et sauvegardez.",
                    "In the “👤 Users” tab click “Change profile photo”, pick a file and save.",
                    "En “👤 Usuarios”, haz clic en “Cambiar foto de perfil”, elige un archivo y guarda."
                ))
        ...
        with tabs[4]:
            ...
            # On détermine si on a suffisamment d’activité pour afficher les KPIs
            ...
            if has_activity:
                ...
                if data:
                    forms = []
                    for fid, cnt in data:
                        row = cur_form.execute(
                            "SELECT titre FROM formations WHERE id=?", (fid,)
                        ).fetchone()
                        if row is None:
                            # soit on skip, soit on met un titre par défaut
                            # continue
                            titre = t("Formation inconnue","Unknown training","Formación desconocida")
                        else:
                            titre = row[0]
                        forms.append({"titre": titre, "lus": cnt})

                    if forms:
                        df_cp = pd.DataFrame(forms)
                    else:
                ...
                # --- Affichage des 4 graphiques en grille 2×2 ---
                ...
                # Si absolument aucune donnée
                st.info(
                    t("Pas encore d’activité sur votre compte.",
                    "No activity yet.",
                    "Sin actividad aún.")
                )
...
<div class="footer">
  <img src="https://i.postimg.cc/9MVK6hzB/OCP-For-Manager-Logo-2025.png" alt="Logo OCP ForManager" />
